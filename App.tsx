...
export default function App() {
  ...
  // Logika Deteksi Mangkir Otomatis & Generate Alerts
  useEffect(() => {
      ...
      if (latest && latest.nextVisitDate < today) {
        ...
        if (!alreadyAlerted) {
          newAlerts.push({
            ...
            message: `Jadwal Kontrol Terlewati (${latest.nextVisitDate}). Segera hubungi pasien.`,
  ...
  const handleExportSystemData = () => {
    ...
    linkElement.setAttribute('download', `SMART_ANC_DB_${new Date().toISOString().split('T')[0]}.json`);
    linkElement.click();
  ...
  const toggleDailyTask = (patientId: string, taskTitle: string) => {
    setState(prev => {
      ...
      return {
        ...prev,
        userChecklists: { ...prev.userChecklists, [patientId]: { ...currentChecklist, [taskTitle]: newStatus } }
      };
    });
  };
  ...
  const getCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        setFormCoords({ lat: pos.coords.latitude.toFixed(6), lng: pos.coords.longitude.toFixed(6) });
        setIsGettingLocation(false);
        showNotification("Lokasi didapatkan");
      },
      () => { alert("Gagal mendapatkan lokasi."); setIsGettingLocation(false); }
    );
  };
        ...
        <div className="space-y-10 animate-in fade-in duration-700">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
             ...
             <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col justify-between group hover:scale-[1.02] transition-transform">
                <div><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Layanan Terpadu</p><h3 className="text-4xl font-black text-gray-900 mt-2 tracking-tighter">{state.ancVisits.length}</h3></div>
                <TrendingUp className="text-emerald-500 self-end" size={32} />
             </div>
             <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col justify-between group hover:scale-[1.02] transition-transform">
        ...
        <div className="space-y-10 animate-in fade-in duration-700">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
             ...
             <div className="bg-indigo-600 p-10 rounded-[3rem] text-white shadow-xl flex flex-col justify-between">
                ...
                <button onClick={() => handleNavigate('register')} className="mt-8 w-full py-4 bg-white/20 hover:bg-white/30 rounded-xl text-[10px] font-black uppercase transition-all">Registrasi Baru</button>
             </div>
                ...
                <div className="relative z-10">
                  ...
                  <div className="space-y-6">
                    <div className="p-8 bg-white/5 border border-white/10 rounded-[2.5rem] backdrop-blur-xl group hover:bg-white/10 transition-colors">
                        <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] mb-2">Total Pantauan</p>
                        <p className="text-4xl font-black tracking-tighter">{patients.length} <span className="text-sm font-bold opacity-40 uppercase ml-2">Ibu Hamil</span></p>
                    </div>
                  </div>
                </div>
                <div className="relative z-10 space-y-4 pt-10"><button onClick={() => handleNavigate('register')} className="w-full py-6 bg-indigo-600 text-white rounded-[2rem] text-xs font-black uppercase tracking-widest shadow-2xl hover:scale-[1.05] transition-all flex items-center justify-center gap-3"><UserPlus size={16}/> Registrasi Pasien</button></div>
